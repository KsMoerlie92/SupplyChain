...
async function loadFromSP() {
  ...
  setStatus('SharePoint bestand ophalen…');
  try {
    const resp = await fetch(url, { credentials: 'include', headers: { Accept: 'application/octet-stream' } });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}: ${resp.statusText}`);
    if ((resp.headers.get('content-type')||'').includes('text/html'))
      throw new Error('SharePoint stuurde een loginpagina. Zorg dat je ingelogd bent.');
    const buf = await resp.arrayBuffer();
    const wb  = XLSX.read(buf, { type: 'array', cellDates: true });
    const fn_name = url.split('/').pop().split('?')[0];
    const dz = document.getElementById('dz-moeder');
    const fn = document.getElementById('fn-moeder');
    processWorkbook(wb, fn_name, dz, fn);
  } catch(err) {
