/* ── Royal IHC — Shared Navigation + IndexedDB (root page) ── */
(function(){
  ...
  const NAV_ITEMS=[
    {icon:'\uD83D\uDD0D',label:'PO Matcher',        href:'PO-Matcher/',            tip:'Koppel Expediting aan Moederlijst via XLOOKUP'},
    {icon:'\uD83D\uDCE6',label:'Legplan & CIPL',     href:'Legplan/',               tip:'3D container packing, CIPL & merk/labels'},
    {icon:'\uD83D\uDCCB',label:'Itemlijst Validator', href:'Itemlijst-Validator/',   tip:'Valideer & corrigeer Itemlijsten van suppliers'},
    {icon:'\u26A0\uFE0F',label:'DG Overview',         href:'DG-Overview/',           tip:'Dangerous Goods analyse \u2014 IHM, IMDG, EU SRR'},
  ];
  ...
  NAV_ITEMS.forEach(n=>{
    html+=`<a class="nav-item" href="${n.href}" data-tip="${n.tip}"><span class="nav-icon">${n.icon}</span>${n.label}</a>`;
  });
  html+='</div>';
